...
public class GCBus {
    ...
    private ObservableList<GCDevice> devices;
    ...
    public void init() {
        ...
        timer.schedule(new ReadQueueTask(), 0, this.readQueueProcessTime);
        ...
        public void run() {
            ...
            while (!readQueue.isEmpty()) {
                try {
                    ...
                    rr.getCaller().processReadResponse(res);
                    lock = true;
                } catch (InterruptedException ex) {
                    Logger.getLogger(GCBus.class.getName()).log(Level.SEVERE, null, ex);
                } catch (ModbusSlaveException ex) {
